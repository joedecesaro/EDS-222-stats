...
rootdir <- ("~/Documents/Bren/Fall/EDS222/EDS-222-stats")
...
* These variables seem to be positively correlated. Tail length and total length appear to have a stronger correlation than tail length and head length.
...
1. Use the plots from section 1 to write your best estimate of the correlation coefficient between each of the two examples in section 1. 
  (i) the length of a possum's total length and the length of possum tail:0.7
  (ii) the length of a possum's head and the length of possum tail: 0.36
...
* The correlation of total length as a function of tail length is `r cor(x = possum$tail_l, y = possum$total_l)`
* The correlation of head length as a function of tail length is `r cor(x = possum$tail_l, y = possum$head_l)`

```{r}
cor(x = possum$tail_l, y = possum$total_l)
...
* possums start with a total length of 41.04 cm and with every 1 cm increase in tail length the total length increases by 1.24 cm.
* possums start with a head length of 73.24 mm and with every 1 cm increase in tail length the head length increases by 0.5 mm.

3. Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$? 
* We need to be cautious when interpreting the intercepts because it is unlikely that any possum will have a tail length of zero.
...
* Our model can tell us that there is likely a positive correlation between body length and tail length because there is a slope greater than 1 in our model.
...
```{r}
prediction <- possum %>% 
  add_predictions(model = lm(total_l ~ tail_l, data = possum)) %>% 
...
```{r}
ggplot(data = prediction, (aes(x = residuals))) + 
  geom_histogram(bins = 25)
```
The results look relatively normally distributed.
